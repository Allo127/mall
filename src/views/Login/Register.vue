<template>
  <div class="Register">
    ...
    <!-- 注册from表单校验 用户名的方式注册-->
    <div class="user-form" v-if="!isPhoneRegister">
    ...
    <div class="Register-btn">
      <van-button :loading="isRegitser" @click="onRegister" loading-text="注册中..." color="#fd7792" round block :disabled="isDisabled"
    ...
    <!-- 退出注册页面 -->
    <div class="cross-Register" @click="handleBack">
  ...
  export default {
    data() {
      return {
        ...
        phone: "", //手机号
        sms: "", //验证码
        isRegitser: false, //注册loading
        isPhoneRegister: false, //是否为手机号码注册
        validateBtn: "发送验证码"
      }
    },
    created() {
      // this.TispRegisterWay()
    },
    methods: {
      //设置登录、用户数据函数
      ...
      // 处理注册
      onRegister() {
        this.isRegitsering = true //在注册中
        setTimeout(() => {
          this.isRegitsering = false
          // console.log(this.$api.register.registerAction(this.userName,this.password))
          this.$api.register.registerAction(this.userName, this.password).then((
            data
          ) => {
